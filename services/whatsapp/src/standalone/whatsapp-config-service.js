...
import { decrypt } from '@microrealestate/common/dist/utils/crypto.js';
...
class WhatsAppConfigService {
  constructor(options = {}) {
    ...
    this.maxCacheSize = options.maxCacheSize || 100; // Prevent memory leaks
  }
  ...
  async getWhatsAppConfig(realmId) {
    ...
    try {
      ...
      logger.error(`Failed to get WhatsApp config for realm ${realmId}`, {
        error: error.message,
        stack: error.stack,
        realmId
      });
      ...
      const fallbackConfig = this._getConfigFromEnvironment();
      logger.warn(`Using fallback environment config for realm ${realmId}`);
  ...
  _buildConfigFromDatabase(dbConfig) {
    ...
    if (dbConfig.accessToken) {
      try {
        decryptedAccessToken = decrypt(dbConfig.accessToken);
      } catch (error) {
        logger.error('Failed to decrypt WhatsApp access token:', error);
        decryptedAccessToken = ''; // Fallback to empty string
      }
    }
    ...
    return {
      ...
      templates: dbConfig.templates || {
        invoice: {
          name: 'factura2',
          language: dbConfig.defaultLanguage || 'es',
          parameters: [
            'tenantName',
            'period',
            'amount',
