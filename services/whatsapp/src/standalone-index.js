...
import MessageService from './standalone/message-service.js'; // Use updated service
...
// MongoDB connection
const MONGO_URL = process.env.MONGO_URL || 'mongodb://localhost:27017/mredb';

async function connectToMongoDB() {
  try {
    await mongoose.connect(MONGO_URL);
    logger.info('Connected to MongoDB successfully', { mongoUrl: MONGO_URL });
  } catch (error) {
    logger.error('Failed to connect to MongoDB:', error);
    // Don't exit - allow service to run in fallback mode
    logger.warn('WhatsApp service will run in environment-only mode');
  }
}
