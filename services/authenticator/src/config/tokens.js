...
export function validateTokenExpiry(expiry) {
  ...
  switch (unit) {
    case 's': return numValue >= 30 && numValue <= 3600; // 30 seconds to 1 hour
    case 'm': return numValue >= 1 && numValue <= 60;    // 1 minute to 1 hour
    case 'h': return numValue >= 1 && numValue <= 24;    // 1 hour to 24 hours
    case 'd': return numValue >= 1 && numValue <= 30;    // 1 day to 30 days
